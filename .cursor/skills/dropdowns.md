...
A dropdown is a floating menu anchored to a trigger — never a permanent navigation rail. The Enhanced panel is a **rounded** (`radius-xxl`, 24px) surface on `neutral-primary-medium` — **borderless and flat**, reading as a *lighter panel floating over the section* (it sits above the page, below modals). Items sit inside at the smaller `radius-md` — the concentric inner corner of a padded rounded parent, not an exception to the rule. Menus open on click by default, trap nothing, and return focus to the trigger on close.
...
Menu items are rounded (`radius-md`, 5px) — concentric with the panel, like every shell in this 4px system (see `radius.md`).
