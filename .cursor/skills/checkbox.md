...
A checkbox lets users pick any number of independent options. The control is a small 16px box that fills `brand` with a `white` check when selected (a light check, because the deep teal-green brand needs light marks for contrast). Its box stays tight at `radius-xs` (4px) so it reads as a small rounded box, never a radio — and the rounded shell rule applies to the cards and list groups it sits inside as well as the tick box itself. When wrapped in a bordered card or list group, that container takes the signature **rounded** (`radius-xxl`, 24px).
...
|---|---|
| Size | 16 × 16px |
| Background (unchecked) | Same as surface (`#E6E3E0` on a warm greige card/section) |
| Background (checked) | `brand` with check mark `white` (light, for contrast on the red fill) |
| Border | `default` (`#D0CBC7`), 1px (unchecked); `brand` when checked |
| Radius | `radius-xs` |
...
The whole row wrapped in a card: padding `spacing-4`, **`radius-xxl`**, background matching the surface (`#E6E3E0`) with a `default` (`#D0CBC7`) border. A checked row is marked by a `brand` border (not a fill change — the background still matches the surface).
...
A vertical stack matching the surface (`#E6E3E0`) with a `default` (`#D0CBC7`) border around the group: rounded corners at **`radius-xxl`**, with hairline `default` dividers between rows (functional row separators); each row padded `spacing-4`.
...
A full-width selectable card: padding `spacing-4`, background matching the surface (`#E6E3E0`), `default` (`#D0CBC7`) border, hover deepens the border toward `default-strong`, selected takes a `brand` border. The background never changes — only the border. The checkbox sits at the inline start; the content block holds a title + description.
...
- **No `radius-full` (or any rounding) on the tick box** — it uses `radius-xs`; a rounded box reads as a radio. The rounded shell is for the surrounding card/list group, never the control.
