...
A phone input is the Enhanced **field shell** typed for `tel`. When a country selector is attached, the selector and number field **fuse into one rounded (`radius-full`, 999px) control** — one shared `default`-bordered, flat (`elevation-none`) shell, square inner edge — exactly like an input group. Verification (OTP) reuses the per-cell box pattern from `number-input.md`. Country flags always carry a text alternative, and OTP digits always use visible per-cell inputs.
...
| Gap (country ↔ number) | Fused group — no gap; shared `radius-full`, `elevation-none` |
...
A row of 4–6 single-digit fields (the PIN pattern from `number-input.md`): ~40px wide, centered, `spacing-2` gap, **`radius-full`**, 4px `brand-medium` focus ring.
