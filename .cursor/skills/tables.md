...
A table is for genuinely comparative, multi-column data — reach for lists or cards otherwise. The Enhanced table wraps in a **rounded** (`radius-xxl`, 24px) card — **borderless and flat** (`elevation-none`): a `neutral-secondary-soft` header over `neutral-primary` rows divided by hairline `default` lines (the one place a thin rule survives, because dividing data rows is functional, not decorative), generous `spacing-6` cell padding, and `font-size-sm` throughout. Emphasis stays subtle — pick *one* readability cue (zebra, hover, or selection tint), never all three at once.
