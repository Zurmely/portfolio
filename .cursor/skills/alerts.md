...
Block alerts and the inline announcement both wear the signature **rounded** (`radius-full`, 999px) — nothing rounds.

| Variant | Border | Radius |
|---|---|---|
| **Default** | None | `radius-full` |
| **Bordered** | 1px intent border on all sides | `radius-full` |
| **Top accent** | 4px (`spacing-1`) intent border on **top edge only**; no side/bottom border | `radius-full` on bottom corners; top edge square against accent bar |
| **Announcement** | 1px intent border on all sides | `radius-full` (999px, rounded) |
...
| Badge | Intent medium/soft fill; `font-size-sm`; `font-weight-medium`; `radius-full` (999px, rounded); padding `spacing-0-5` vertical, `spacing-2` horizontal |
...
- **No off-scale radius** — every alert, block or announcement, is rounded (`radius-full`, 999px); nothing rounds.
