...
The card is the face of Enhanced: a clean surface with a **hairline `default` (`#D0CBC7`) border**, **no shadow**, the signature **rounded** (`radius-xxl`, 24px) corners, and generous `spacing-6` padding. A card always carries the **same background color as the section it sits on** — `#E6E3E0` / `#E0DCD8` on content sections, `#161616` on the hero/footer — so separation comes from the border alone, never a lighter fill and never a drop shadow. **The one exception is a dashboard widget, which fills `surface-raised` (`#ECECEC`)** (see `colors.md`). Cards do the bulk of the layout work — articles, products, profiles, pricing, dashboards — so they stay calm and let their content lead.
...
|---|---|
| Background | Same as the section it sits on — `neutral-secondary-soft` (`#E6E3E0`) or `neutral-tertiary` (`#E0DCD8`) on content sections; `surface-dark` (`#161616`) on the hero/footer |
| Border | 1px — the only separation from the section: `default` (`#D0CBC7`) on greige sections, **`border-dark` (`#2C2C2C`) on a `#161616` section** |
...
Every variant is the same shell — bordered, shadowless, section-aware surface, rounded corners — rearranged around its content.

### Default

Title + body; the whole card may be a single link.

### Feature card / feature grid item

An icon over a heading and a line of copy — used in **features / benefits / services / "how it works"** sections, in a card or a bare grid cell.

| Part | Value |
|---|---|
| **Icon** | **32 × 32px — always** (width AND height). Outline style, `heading` or `body` colour. **Never the 16–20px default UI icon** — a feature icon under 32px is a bug. |
| Heading | 24px (the `feature-heading` role, `typography.md`), `spacing-4` below the icon |
| Body | `font-size-lg`+ (≥ 18px landing floor), `body` colour |
| Layout | Icon → heading → copy, stacked; cards match the section background with the `default` border, or a bare grid item reuses the section side-rail dividers |
...
Tier name, price, feature list, and CTA — the highlighted tier is marked by a `brand` border (in place of the default `#D0CBC7`), never a different fill; its background still matches the section.
...
Cards are flat — separation is read from the `default` (`#D0CBC7`) border alone, never a shadow or a fill shift. The card background always equals its section.

| State | Surface |
|---|---|
| Resting | Same background as the section (`#E6E3E0` / `#E0DCD8` on content sections, `#161616` on hero/footer) + `default` border |
...
- **No corners other than `radius-xxl`** (4px, rounded) and no raw hex — a card that is sharp-cornered or pill isn't an Enhanced card.
- **No shadows on cards** — they are flat; separation comes from the `default` border and surface tone, not `elevation`.
- **No borderless cards** — every card carries the hairline `default` (`#D0CBC7`) border; do not drop it and rely on a shadow.
- **No card fill that differs from its section** — the card background always equals the section it sits on (`#E6E3E0` on content, `#161616` on hero/footer); separation is the `default` border alone, never a lighter, derived, or tinted fill. Emphasis uses a `brand` border, not a fill change.
- **No heavy or colored card borders** — the border is the single hairline `default` (`#D0CBC7`); do not thicken it or swap it for an accent unless a status card calls for an intent edge.
