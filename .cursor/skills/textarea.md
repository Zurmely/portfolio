...
A textarea is the Enhanced **field shell** from `input-field.md` grown to multiple lines — the same surface-matching fill with a `default` border, flat (`elevation-none`), and brand focus ring, but a **`radius-xl` (16px) corner instead of the field's pill** (a 999px `radius-full` bows a tall multi-line box), with `rows` setting the min height. Richer patterns (WYSIWYG, comment box, chat) wrap the field in a single rounded composite card: the inner textarea goes borderless and the **focus ring moves to the wrapper**, so the whole composite lights up as one control rather than ringing a box inside a box.
...
|---|---|
| Shell | Same border, background, shadow as `input-field.md`, but **`radius-xl` (16px)** — never the pill (`radius-full`) single-line fields use |
...
| Part | Token / value |
|---|---|
| Outer wrapper | `default` (`#D0CBC7`) border, `radius-xxl`, background matching the surface (`#E6E3E0`), flat (`elevation-none`) |
...
| Text control | Flex-grow; single-line height with `rows="1"`; shell matching the surface (`#E6E3E0`) with a `default` border, `radius-xxl`, padding `spacing-2-5` `spacing-3` |
...
- **No borderless textarea outside a defined composite** — always a shell border at `radius-xl` (16px) on a bare textarea, or a wrapper border at `radius-xxl` (24px) on a composite.
- **No pill on a textarea** — a multi-line box is `radius-xl` (16px), never `radius-full`; the pill is for single-line fields only.
