...
A drawer slides in from a viewport edge — unlike a modal, it's anchored to that edge and often keeps wayfinding context in view. It shares the modal's `surface-raised` (`#ECECEC`) raised-panel surface and is **borderless and flat** (`elevation-none`) — separated from the page by that raised surface and the backdrop scrim — but its corners follow the **edge-anchored exception** in `radius.md`: edges flush against the viewport stay square (`radius-none`), and only a free inward edge takes the `radius-xxl` (24px) shell. A full-height side drawer is therefore square on its flush edges by design.
...
|---|---|
| Panel background | `surface-raised` (`#ECECEC`) — raised panel over the scrim |
| Panel border | None — the lighter panel surface and the backdrop scrim separate it from the page |
| Panel radius | **Square on edges flush to the viewport**; the inward free edge may take `radius-xxl` (24px). A full-height side drawer stays square — see the edge-anchored exception in `radius.md` |
...
- **No rounding an edge flush to the viewport** — edges flush to the viewport stay square (`radius-none`); only a free inward edge takes the `radius-xxl` (24px) shell.
