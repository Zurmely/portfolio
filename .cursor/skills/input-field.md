...
|---|---|
| Width | 100% of parent |
| Background | Same as its surface (`neutral-primary-soft` `#E6E3E0` on a warm greige card/section) |
| Border | `default` (`#D0CBC7`), 1px — the resting edge that defines the field |
...
**Required composite behavior:** when any child of the shell is focused, the outer shell takes the `brand` border and the `brand-medium` focus ring; the inner control shows no outline and no ring of its own. Never put a focus ring directly on a nested input inside a composite shell — that produces a broken ring around the text/placeholder only.
...
| Row border | `default` (`#D0CBC7`) on the outer perimeter |
| Row radius | `radius-full` on outer corners only |
| Addon background | Same as the field (matches the surface) |
| Addon border | A `default` (`#D0CBC7`) divider on the shared edge separates addon from control |
| Addon padding | `spacing-3` inline |
| Addon text | `font-size-sm`, `body` |
| Addon icon | 16 × 16px, `body` |
| Field inner radius | Flush on shared edge; outer corner keeps `radius-full` |
...
Query fields use the same **field shell** with a leading magnifying glass — same rounded (`radius-full`, 999px), same surface-matching fill with a `default` border, same flat (`elevation-none`) surface, same brand focus ring on the **wrapper** via `:focus-within`. Search never invents its own look; it only adds search affordances (leading icon, optional clear/voice/submit, optional scope menu). The search action is always keyboard-reachable, and any scope dropdown is drawn by `dropdowns.md`, not styled inline.
...
A composite row: the text field plus an adjacent scope trigger (e.g. "All categories"). The menu uses flat, `radius-full`, and `dropdowns.md` item spacing; gap between field and trigger is `spacing-2`.
...
- **No divergent field shell** — same border, rounded (4px) radius, and elevation as the standard field shell.
...
|---|---|---|---|---|
| Default | Same as surface (`#E6E3E0`) | `default` (`#D0CBC7`) | `heading` / `body` | `brand-medium` |
...
- **No corners other than `radius-full`** on the shell — rounded (4px) is the signature; a fused input group rounds its outer corners and stays square on the inner shared edge.
- **No shadow on a resting input** — the field is flat (`elevation-none`); contrast comes from the fill.
- **No contrasting resting fill** — the field background matches its surface; the `default` (`#D0CBC7`) border defines the shell, and focus swaps it to a `brand` edge.
