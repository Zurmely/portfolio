...
The rounded corner lives on the outermost shell; interior items square off so the rounding reads only at the edges of the group.

### Variant: grouped (default)

| Element | Border | Radius |
|---|---|---|
| Root | None — lighter card surface defines the group | `radius-xxl` (24px) on outer corners |
...
- **No outer shell below `radius-xxl`** — the rounded frame is the signature; interior items square off, they don't shrink the corner.
