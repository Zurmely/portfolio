...
| Border | `default` (`#D0CBC7`), 1px (unchecked); `brand` when selected |
| Radius | `radius-full` |
| Inner dot (checked) | 8 × 4px circle, `brand` fill, centered |
...
A row card: padding `spacing-4`, **`radius-xxl`**, background matching the surface (`#E6E3E0`) with a `default` (`#D0CBC7`) border. Selected is marked by a `brand` border (not a fill change — the background still matches the surface).

### Radio list group

A vertical grouped list matching the surface (`#E6E3E0`) with a `default` (`#D0CBC7`) border around the group (rounded **`radius-xxl`** corners) and hairline `default` row dividers. The section heading above sits at `font-size-base`, `font-weight-semibold`, `heading`, `spacing-3` below.
