...
> The type system for **TypeUI · Enhanced**. Enhanced speaks in **PP Neue Montreal** — a modern editorial grotesque. Body copy is set at a generous **20px, `font-weight-medium` (500), in `body` (`#727272`)**, over a compact **14px control baseline** (buttons, inputs, dense app chrome), with a disciplined heading ramp in `heading` (`#1E1E1E`) and tight negative tracking on display sizes — so pages read confident and editorial, never cramped. Sizes, weights, line heights, letter spacing, and family stacks are literal values and the single source of truth; components reference these tokens (and color tokens from `colors.md`), never ad-hoc type settings.
...
Default body: **`font-size-xl` (20px)** + **`line-height-body`** + **`font-weight-medium` (500)** + **`font-family`**, in **`body` (`#727272`)**. (Dense dashboard/app body may step down to `font-size-sm` (14px) — see the application role table.)
...
| Token | Value |
|---|---|
| font-family | "PP Neue Montreal", "Neue Montreal", "Inter", ui-sans-serif, system-ui, -apple-system, "Segoe UI", Roboto, Helvetica, Arial, sans-serif |

**Brand typeface for this design system:** PP Neue Montreal — a modern grotesque; [Inter](https://fonts.google.com/specimen/Inter) is the web-safe fallback. Load the face in your product’s font layer; the token above is the stack components reference.
...
| line-height-display | 1 | **Display-scale headings only** — rendered font size **strictly greater than 46px** (see rule below) | Matches font size |
...
**Default body pairing:** `font-size-xl` + `font-weight-medium` + `line-height-body` → 20px / 30px line box. (Dense app/dashboard body steps down to `font-size-sm` → 14px / 21px.)

**Display line-height rule (mandatory):** **`line-height-display` (1) is reserved for text and headings whose rendered size is strictly greater than 46px** — typically `font-size-9xl` (50px), `font-size-10xl` (60px), and `font-size-hero` (72px) on desktop, plus marketing hero h1 at full display scale. **Anything at 46px or below must not use `line-height-display`.** Use the normal role tokens instead:

| Rendered size | Line height token | Multiplier |
|---|---|---|
| **> 46px** (display / hero only) | `line-height-display` | 1 |
| **≤ 46px** — headings & titles | `line-height-heading` | 1.3 |
| **≤ 46px** — body, lead, paragraphs | `line-height-body` | 1.5 |
| **≤ 46px** — labels, controls, captions | `line-height-component` or `line-height-detail` | 1.3 |

This follows TypeUI fundamentals: tight 1:1 leading is a **display-scale** treatment only; sub-46px type keeps readable normal leading (`line-height-heading` or `line-height-body`) — never crushed to 1.

**Token cutoff (desktop):** `font-size-8xl` (45px) and every smaller step use normal leading; `font-size-9xl` (50px) and above may use `line-height-display` when the element is display-scale (hero h1, oversized marketing display — not app chrome, card titles, or section headings at `font-size-7xl` / 40px).
...
| **Underline** | Links **on hover only** — links are **not** underlined by default; the underline appears on hover. Never for emphasis; never on nav / sidebar links (see `SKILL.md`) |
...
- **`line-height-display` (1) only above 46px** — hero h1 and display-scale marketing type above 46px rendered size; everything at 46px or below uses `line-height-heading` (1.3) or `line-height-body` (1.5) per role.
- Marketing pages must not reuse app-sized h1 tokens on hero bands; app pages must not reuse `font-size-hero` on page titles.

---

## Minimum text size — landing & ecommerce

On **landing / marketing and ecommerce** pages, **no text renders below `font-size-lg` (18px)** — body (20px), leads (22px), links, list items, in-flow captions, and paragraphs all sit at **18px or larger**.

- **Exempt — microcomponents:** badges, alerts, chips / tags, and similar small status labels may go below 18px.
- **Exempt — dashboards & application UI:** this floor is **landing / ecommerce only**. Dashboard and app surfaces keep the dense scale (11–14px labels, captions, table text) — the floor never applies there.
...
| section-heading | font-family | font-size-7xl (40px) | font-weight-semibold | line-height-heading | `heading` |
| feature-heading | font-family | 24px | font-weight-semibold | line-height-heading | `heading` |
| lead | font-family | font-size-2xl (22px) | font-weight-normal | line-height-body | `body` |
| body | font-family | font-size-xl (20px) | font-weight-medium | line-height-body | `body` |
| overline | font-family | font-size-xs (12px) | font-weight-medium | line-height-detail | letter-spacing-wider | `body-subtle` |

### Link sizes by surface

Navigation / chrome links are sized by where they live:

| Surface | Link size |
|---|---|
| **Navbar** | **`font-size-xl` (20px)** |
| **Footer** | **`font-size-xl` (20px)** |
| **Banner** (announcement / promo bar) — **all text, copy + links** | **`font-size-md` (16px)** |
| **Dashboard / application** | **`font-size-md` (16px)** |

Only **navbar and footer** links are 20px; **banner** and **dashboard** links are the normal 16px. (Inline links inside body copy inherit the surrounding text size.)
...
|---|---|
| Marketing hero h1 | hero-h1 → font-size-hero (≤72px) + line-height-display (rendered size > 46px) |
| Marketing section title | section-heading → font-size-7xl + line-height-heading |
| App / dashboard page h1 | app-h1 → font-size-4xl (≤28px) |
| Card / widget title | widget-title → font-size-xl |
| Paragraphs | body → font-size-xl (20px), font-weight-medium (dense app body → font-size-sm) |
...
- **No `line-height-display` at 46px or below** — only display-scale type strictly greater than 46px rendered size may use `line-height-display` (1); all smaller headings use `line-height-heading` (1.3), body uses `line-height-body` (1.5).
- **No `line-height-display` on section titles, card titles, stats, or app chrome** — those surfaces stay at or below 46px and use normal leading per role.
- **No gap other than 24px below a heading or paragraph** — a heading or paragraph followed by anything (paragraph, buttons, card, list, image, or any block) keeps **exactly** `spacing-6` (24px) below it — never more, never less.
