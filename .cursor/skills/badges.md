...
| Part | Role |
|---|---|
| **Root** | Inline-flex rounded chip (`radius-full`, 999px). **Width is always auto** — hugs label content; never full width |
...
Two text sizes plus icon-only boxes — and every one is rounded.

| Size | Type | Padding | Radius |
|---|---|---|---|
| **Small (default)** | Text | `spacing-1-5` horizontal, `spacing-0-5` vertical | `radius-full` (999px, rounded — always) |
| **Large** | Text | `spacing-2` horizontal, `spacing-1` vertical | `radius-full` (999px, rounded — always) |
| **Icon-only small** | Rounded | — | 20 × 20px box, `radius-full` (999px) |
| **Icon-only large** | Rounded | — | 24 × 24px box, `radius-full` (999px) |
...
Every variant below is rounded — the differences are fill, border, and contents, never the corner.

### Default

Soft fill, no border. Inline-flex, **`width: auto`** — the chip hugs its label; never `width: 100%`, never stretch to fill a row or card.

### Bordered

A 1px intent border on the full perimeter; same padding and type as default.

### Rounded shape (always on)

Every badge in Enhanced is **rounded** — `radius-full` (999px) on the root in all sizes and variants (default, bordered, link, icon, dismissible, count). There is no pill or sharp-cornered badge in this theme; only the inner status dot and avatar image are circular.

### Large bordered (inset ring)

Large size with a 1px inset ring in the intent border color (`brand-subtle`, etc.) on the rounded `radius-full` shell — reads like bordered without doubling the outer edge.
...
A fixed rounded/circle box with a centered icon and no label — so an `aria-label` describing its meaning is required.
...
| Radius | `radius-full` (999px, rounded) |
| Position | Top-trailing corner of parent, slight negative inset |

### Button-attached count

A rounded count inside a button's label area: ~18px box, `brand-soft` fill, `fg-brand-strong` text, `spacing-2` gap after the label.
...
- **No rounded badges** — every badge is rounded (`radius-full`, 999px); never `radius-md`, `radius-full`, or pill corners. This is the signature, not a preference. (The inner status dot and avatar image stay circular.)
...
- **No full-width badges** — badges are always **`width: auto`** (content-sized). Never stretch a badge to 100% of a container, grid cell, or flex row; use `align-self: flex-start` or `inline-flex` in stacked layouts so the chip stays only as wide as its label.
